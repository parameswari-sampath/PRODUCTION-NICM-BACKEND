...
// Test data - 5 MCQ responses (same as individual test for fair comparison)
...
export default function () {
  const response = http.post(
    'http://localhost:8080/api/load-test/batch',
    payload,
    params
  );
...
export function handleSummary(data) {
  return {
    'batch-test-results.json': JSON.stringify(data, null, 2),
...
function textSummary(data, options) {
  ...
  let summary = '\n';
  summary += indent + '========== BATCH INSERT TEST RESULTS ==========\n\n';
